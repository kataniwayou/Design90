...
public class OrchestrationService : IOrchestrationService
{
    ...
    public async Task StartOrchestrationAsync(Guid orchestratedFlowId)
    {
        ...
        try
        {
            ...
            var assignmentManagerTask = _managerHttpClient.GetAssignmentManagerDataAsync(orchestratedFlowId);
            ...
            await ValidateAssignmentSchemas(assignmentManagerData);

            // Step 3: Find and validate entry points
            activity?.SetTag("step", "3-FindAndValidateEntryPoints");
            var entryPoints = FindEntryPoints(stepEntities);
            ...
            var terminationPoints = FindTerminationPoints(stepEntities);
            ...
            ValidateCircularWorkflow(stepEntities, terminationPoints);
    ...
    public async Task<OrchestrationStatusModel> GetOrchestrationStatusAsync(Guid orchestratedFlowId)
    {
        ...
        catch (Exception ex)
        {
            ...
            _metricsService.RecordException(ex.GetType().Name, "error", isCritical: false, Guid.Empty);
    ...
    private List<Guid> FindEntryPoints(Dictionary<Guid, StepNavigationData> stepEntities)
    {
        _logger.LogDebugWithCorrelation("Finding entry points from {StepCount} steps",
            stepEntities.Count);
    ...
    private List<Guid> FindTerminationPoints(Dictionary<Guid, StepNavigationData> stepEntities)
    {
        ...
        foreach (var kvp in stepEntities)
        {
            if (kvp.Value.NextStepIds == null || kvp.Value.NextStepIds.Count == 0)
            {
                ...
                _logger.LogDebugWithCorrelation("Found termination point: {StepId}", kvp.Key);
            }
        }
    ...
    private void ValidateCircularWorkflow(Dictionary<Guid, StepNavigationData> stepEntities, List<Guid> terminationPoints)
    {
        _logger.LogDebugWithCorrelation("Validating circular workflow from {StepCount} steps", stepEntities.Count);
        ...
        _logger.LogDebugWithCorrelation("Found {TotalNextSteps} total next step references", allNextSteps.Count);
        ...
        {
            _logger.LogInformationWithCorrelation("Circular workflow validation passed. No steps appear multiple times as next steps");
            return;
        }

        _logger.LogDebugWithCorrelation("Found {DuplicateStepCount} steps that appear multiple times as next steps: {DuplicateSteps}",
        ...
        if (circularSteps.Count > 0)
        {
            ...
            _logger.LogErrorWithCorrelation("Circular workflow validation failed: {ErrorMessage}", errorMessage);
            throw new InvalidOperationException(errorMessage);
        }

        _logger.LogInformationWithCorrelation("Circular workflow validation passed. All duplicate steps ({DuplicateStepCount}) are termination points",
    ...
    private async Task ValidateAssignmentSchemas(Dictionary<Guid, List<AssignmentModel>> assignments)
    {
        ...
        _logger.LogInformationWithCorrelation("Starting assignment schema validation for {Count} entities.", allAssignments.Count);
        ...
        var validationTasks = allAssignments.Select(async assignment =>
        {
            try
            {
                ...
                {
                    await ValidatePayloadAgainstSchema(addressModel.Payload, addressModel.SchemaId, "Address", addressModel.Name, addressModel.Version);
                }
                else if (assignment is DeliveryAssignmentModel deliveryModel)
                {
                    await ValidatePayloadAgainstSchema(deliveryModel.Payload, deliveryModel.SchemaId, "Delivery", deliveryModel.Name, deliveryModel.Version);
                }
                else if (assignment is PluginAssignmentModel pluginModel)
                {
                    await ValidatePayloadAgainstSchema(pluginModel.Payload, pluginModel.SchemaId, "Plugin", pluginModel.Name, pluginModel.Version);
                }
            }
            catch (Exception ex)
            {
                _logger.LogErrorWithCorrelation(ex, "Schema validation failed for assignment. EntityId: {EntityId}, Type: {Type}",
        ...
        _logger.LogInformationWithCorrelation("Assignment schema validation completed successfully for {Count} entities.", allAssignments.Count);
    }
    /// <summary>
    ...
    private async Task ValidatePayloadAgainstSchema(string payload, Guid schemaId, string entityType, string name, string version)
    {
        // Skip validation if SchemaId is empty (optional validation)
        if (schemaId == Guid.Empty)
        {
            _logger.LogDebugWithCorrelation("Schema validation skipped for {EntityType} entity - SchemaId is empty. Name: {Name}, Version: {Version}",
                entityType, name, version);
            return;
        }

        var schemaDefinition = await _managerHttpClient.GetSchemaDefinitionAsync(schemaId);
        ...
        _logger.LogDebugWithCorrelation("Schema validation passed for {EntityType} entity. Name: {Name}, Version: {Version}",
            entityType, name, version);
    }
