...
public class ActivityFailedEventConsumer : IConsumer<ActivityFailedEvent>
{
    ...
    public async Task Consume(ConsumeContext<ActivityFailedEvent> context)
    {
        ...
        try
        {
            ...
            _flowMetricsService.RecordCommandConsumed(success: true, activityEvent.OrchestratedFlowEntityId, activityEvent.StepId, activityEvent.ExecutionId, Guid.Empty);
        ...
        catch (Exception ex)
        {
            ...
            _flowMetricsService.RecordCommandConsumed(success: false, activityEvent.OrchestratedFlowEntityId, activityEvent.StepId, activityEvent.ExecutionId, Guid.Empty);
    ...
    private async Task ProcessSingleNextStepAsync(ActivityFailedEvent activityEvent, Guid nextStepId, OrchestrationCacheModel orchestrationData)
    {
        ...
        try
        {
            ...
            if (!shouldExecuteStep)
            {
                _logger.LogInformationWithCorrelation("Skipping step due to entry condition. NextStepId: {NextStepId}, EntryCondition: {EntryCondition}, ActivityStatus: {ActivityStatus}",
                    nextStepId, nextStepEntity.EntryCondition, "Failed");
                return;
            }
            ...
            _flowMetricsService.RecordEventPublished(success: true, activityEvent.OrchestratedFlowEntityId, nextStepId, activityEvent.ExecutionId, Guid.Empty);
        ...
        catch (Exception ex)
        {
            ...
            _flowMetricsService.RecordEventPublished(success: false, activityEvent.OrchestratedFlowEntityId, Guid.Empty, activityEvent.ExecutionId, Guid.Empty);
    ...
    private async Task CopyCacheProcessorDataAsync(ActivityFailedEvent activityEvent, Guid nextStepId, Guid destinationProcessorId)
    {
        ...
        try
        {
            ...
            var destinationKey = _rawCacheService.GetProcessorCacheKey(destinationProcessorId, activityEvent.OrchestratedFlowEntityId, activityEvent.CorrelationId, activityEvent.ExecutionId, nextStepId, Guid.NewGuid());
